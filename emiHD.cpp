...
int main()
{
    ...
    // Loading Fonts goes here

    // State variables before main loop
...
static void CreateMainMenuBar()
{
    if (ImGui::BeginMainMenuBar())
    {
        ...
        {
            if (ImGui::MenuItem("Placeholder", NULL, false, false)) {}
            ImGui::EndMenu();
        }
...
static void CreateParentWindow() {
    ...
    static ImGuiWindowFlags flags = ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoDecoration;
    // I want to use the "work area"
    const ImGuiViewport* viewport = ImGui::GetMainViewport();
    ImGui::SetNextWindowPos(viewport->WorkPos);
    ImGui::SetNextWindowSize(viewport->WorkSize);

    if (ImGui::Begin("Root/Parent Window", NULL, flags))
    {
...
static void CreatePanelContainers() {
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_None;
    ImGui::PushStyleVar(ImGuiStyleVar_ChildRounding, 2.5f);
    ImGui::BeginChild("ChildL", ImVec2(ImGui::GetContentRegionAvail().x * 0.25f, 0), true, window_flags);
    ImGui::Text("Left Panel ");
    // Timer Panel
    {
        ImGui::BeginChild("TimerPanel", ImVec2(0, ImGui::GetContentRegionAvail().y * 0.75f), true, window_flags);
        ImGui::Text("Timers Panel");
        // Timers
        {
            auto time = std::chrono::duration_cast<std::chrono::milliseconds>(g_appStart.time_since_epoch());
            char buf[255];
            sprintf(buf, "Application started at: %d", (int)time.count());
            ImGui::Text(buf);
            auto delta = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - g_appStart);
            sprintf(buf, "Time elapsed: %d", (int)delta.count());
            ImGui::Text(buf);
        }
        ImGui::EndChild();
    }
    // Widgets Panel
    {
        ImGui::BeginChild("WidgetsPanel", ImVec2(0, 0), true, window_flags);
    ...
    ImGui::BeginChild("ChildR", ImVec2(ImGui::GetContentRegionAvail().x, 0), true, window_flags);
